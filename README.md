...
- As can be seen from some of the script names above (e.g., `22_rscript_ccsel_main.R`), quite a few scripts use the `R` programming language (`v3.6.0` or greater) and concomitant packages. An in-exhaustive list of packages needed includes: `dplyr`, `tidyr`, `stringr`, `magrittr`, `seqinr`, `data.table`, `Biostrings`, `protr`, `purrr`, `ape`, `taxize`, `httr`, and `seqvisr` (from `GitHub`). Accessing the scripts through RStudio should highlight missing packages, and ease their installation (with the exception of Bioconductor packages).
...
- `ncbi_submission` contains material concerning the data (raw reads, transcriptomes) submitted to NCBI. These can be found on NCBI under the accession code `PRJNA824716`.
